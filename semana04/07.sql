...
CREATE TABLE IF NOT EXISTS pedidos ( -- Le decimos acá que cree la tabla SI NO EXISTE
	id SERIAL PRIMARY KEY,
	total NUMERIC(2),
	fecha DATE DEFAULT CURRENT_DATE,
	cliente_id INT REFERENCES clientes(id) -- Asi se crea la relación en la misma definición de columna
	-- Si queremos crear la CONSTRAINT AL FINAL
	-- CONSTRAINT fk_cliente REFERENCES
);

-- CONTROL + ENTER > Ejecuta la sentencia seleccionada o la sentencia donde se ubica el cursos
-- CONTROL + SHIFT + O > Muestra/Oculta la salida del servidor
...
-- cambiamos el tipo de dato en total
-- NUMERIC > puede recibir 2 parámetros, en el cual si solo se pasa 1 será la cantidad de enteros
-- Y si se le pasa 2, el primer parámetro será la cantidad de enteros y el segundo la cantidad de decimales
...
-- Visualizar la data de 2 o más tablas
-- INNER JOIN (intersección ó combinación interna)
SELECT * FROM clientes INNER JOIN pedidos ON clientes.id = pedidos.cliente_id;

-- Insertamos un pedido sin cliente
INSERT INTO pedidos (total, fecha, cliente_id) VALUES
(999.99, DEFAULT, NULL);
...
-- Ejercicios
-- 1. Listar el nombre del cliente y el total de cada uno de sus pedidos
SELECT c.nombre, p.total 
FROM clientes c INNER JOIN pedidos p ON c.id = p.cliente_id;

-- 2. Listar todos los clientes con sus pedidos, aún así no tengan ningún pedido
...
-- 4. Encuentra los productos que nunca se han vendido (no están presentes en detalle_pedidos)
SELECT p.nombre
FROM productos p LEFT JOIN detalle_pedidos dp ON p.id = dp.pedido_id
WHERE dp.id IS NULL;
